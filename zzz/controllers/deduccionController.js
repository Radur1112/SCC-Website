...
module.exports.crear = async (req, res, next) => {
  try {
    ...
    let crearDatos = {
        ...
        idTipoDeduccion: datos.idTipoDeduccion ?? datos.idTipo,
...
module.exports.crearMultiples = async (req, res, next) => {
  try {
    ...
    const deduccionsPromises = datos.map(async (dato) => {
      ...
      let crearDatos = {
        ...
        idTipoDeduccion: dato.idTipoDeduccion ?? dato.idTipo,
    ...
    Promise.all(deduccionsPromises)
      .then((results) => {
        res.status(201).json({
          success: true,
