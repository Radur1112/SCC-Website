...
module.exports.getPendientesByIdSupervisor = async(req, res, next) => {
  try {
    ...
    const data = await db.query(`
      ...
      FROM ${nombreTabla} i
      INNER JOIN usuariosupervisor us ON us.idSupervisor = ?
      INNER JOIN usuario u ON u.id = us.idUsuario AND u.id = i.idUsuario
      WHERE i.estado = 2 AND us.idUsuario = i.idUsuario
